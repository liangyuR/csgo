...
"""
主題顏色定義模組
定義 GUI 中所有元素在亮色和暗色主題下的顏色
支援從 theme_colors.json 載入自定義顏色
"""
...
class ColorConfigLoader:
            ...
            for path in possible_paths:
                ...
                if os.path.exists(abs_path):
                    with open(abs_path, 'r', encoding='utf-8') as f:
                        ...
                        print(f"[ThemeColors] 已載入自定義顏色配置: {abs_path}")
                        return
                        
        except Exception as e:
            print(f"[ThemeColors] 載入顏色配置失敗，使用預設值: {e}")
    ...
    def get_color(self, *keys, default_light: str = "#000000", default_dark: str = "#FFFFFF") -> Tuple[str, str]:
        ...
        Args:
            *keys: 配置路徑的鍵，例如 ("primary_colors", "primary")
            default_light: 預設亮色值
            default_dark: 預設暗色值
        
        Returns:
            (light_color, dark_color) 元組
        """
        try:
    ...
    def get_rgba_color(self, *keys, 
        ...
        Args:
            *keys: 配置路徑的鍵
            default_light: 預設亮色 RGBA 值
            default_dark: 預設暗色 RGBA 值
        
        Returns:
            (light_rgba, dark_rgba) 元組
        """
        try:
            value = ColorConfigLoader._config
            for key in keys:
...
class ColorPair:
    """亮色和暗色主題的顏色對"""
    light: str  # 亮色主題色號 (HEX 格式)
    dark: str   # 暗色主題色號 (HEX 格式)
