# xbox_controller.py - Xbox 360 手把模擬模組
"""
使用 vgamepad 函式庫模擬 Xbox 360 控制器右搖桿
將滑鼠移動 (dx, dy) 轉換為右搖桿輸入，實現遊戲內的視角控制

原理：
- 建立虛擬 Xbox 360 控制器 (透過 ViGEmBus 驅動)
- 將 AI 計算的滑鼠移動量映射為右搖桿 (Right Stick) 的偏移
- 支援靈敏度調整、死區設定、反應曲線等參數
"""
...
def _launch_vigem_installer_and_exit() -> None:
    """啟動 ViGEmBus 安裝程式並結束當前程序"""
    if os.path.exists(_VIGEM_INSTALLER):
        import subprocess
        print("[Xbox] 未偵測到 ViGEmBus 驅動，正在啟動安裝程式...")
        try:
            subprocess.Popen([_VIGEM_INSTALLER], shell=False)
        except Exception as launch_err:
            print(f"[Xbox] 無法啟動安裝程式: {launch_err}")
    else:
        print(f"[Xbox] 找不到 ViGEmBus 安裝程式: {_VIGEM_INSTALLER}")
        print("[Xbox] 請至 https://github.com/nefarius/ViGEmBus/releases 手動下載安裝")

    print("[Xbox] 安裝完成後請重新啟動 Axiom。程式即將關閉...")
    time.sleep(2)
    os._exit(0)

# vgamepad 採用 lazy import，僅在 connect() 被呼叫時才載入
# 避免在模組匯入時觸發 ViGEmBus 驅動檢查
vg = None


def _import_vgamepad():
    """嘗試匯入 vgamepad，若 ViGEmBus 未安裝則啟動安裝程式並結束程序"""
    global vg
    if vg is not None:
        return True
    try:
        ...
        print("[Xbox] vgamepad 未安裝，請執行: pip install vgamepad")
        return False
    except Exception as e:
        # ViGEmBus 驅動未安裝（例如 VIGEM_ERROR_BUS_NOT_FOUND）
        print(f"[Xbox] vgamepad 載入失敗: {e}")
...
class XboxController:
    ...
    使用 vgamepad 建立虛擬 Xbox 360 控制器，
    並將滑鼠移動量映射到右搖桿輸入。
    
    Attributes:
        sensitivity: 靈敏度倍率 (預設 1.0)
        deadzone: 死區閾值，低於此值的輸入會被忽略 (預設 0.05)
        stick_duration: 搖桿輸入持續時間（秒）(預設 0.03)
        max_stick_value: 搖桿最大映射值 (0.0~1.0) (預設 1.0)
    """
